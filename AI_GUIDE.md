...
├── tokens/
│   ├── base.css            # 토큰을 CSS 변수로 변환 (라이트/다크 모드 포함)
│   └── campaign.css        # 캠페인 전용 토큰
├── styles/
│   ├── sections.css        # 모든 섹션 스타일
│   ├── components.css      # pl-btn, pl-card 등 공통 컴포넌트 스타일
│   └── dark-mode.css       # 다크 모드 오버라이드 (섹션별 커스텀)
├── scripts/
│   ├── components.js       # Web Components 정의 (pl-section-title, pl-button, pl-card)
│   ├── generate-tokens.js  # PLtokens.json → base.css 변환 스크립트
│   ├── validate-tokens.js  # CSS 변수 참조 유효성 검사
│   ├── new-section.js      # 새 섹션 HTML 스캐폴딩 CLI
│   └── validate-sections.js # 섹션 HTML 구조 표준 검증기
├── sections/
│   ├── hero/               # Hero 섹션 HTML 파일들
│   ├── about/              # About 섹션 HTML 파일들
│   ├── benefit/            # Benefit 섹션 HTML 파일들
│   ├── review/             # Review 섹션 HTML 파일들
│   ├── cta/                # CTA 섹션 HTML 파일들
│   ├── stats/              # Stats 섹션 HTML 파일들
│   ├── faq/                # FAQ 섹션 HTML 파일들
│   ├── form/               # Form 섹션 HTML 파일들
│   ├── gnb/                # GNB 섹션 HTML 파일들
│   └── etc/                # 기타 섹션 HTML 파일들
...
npm run tokens:sync      # 토큰 재생성 + 검사 한 번에
npm run new-section      # 새 섹션 HTML 파일 스캐폴딩
                         # 예: npm run new-section -- --type=about --name=type-g-video
npm run validate         # 모든 섹션 HTML 구조 표준 검증
                         # 예: npm run validate -- --file=sections/about/type-f-image.html
```
...
다크 모드 스타일은 **두 곳**에 분산되어 있습니다:

| 파일 | 내용 |
|------|------|
| `tokens/base.css` | 시맨틱 컬러 토큰 오버라이드 (`--pl-text-*`, `--pl-bg-*` 등 자동 전환) |
| `styles/dark-mode.css` | 섹션별 커스텀 오버라이드 (토큰만으로 해결 안 되는 케이스) |

**규칙**: 가능하면 시맨틱 토큰 사용으로 자동 처리. 섹션별 특수 케이스만 `dark-mode.css`에 추가.
...
⚠️ **주의**: 인라인 `<style>` 태그 안에 `[data-theme="dark"]` 규칙을 작성하지 마세요. 반드시 `dark-mode.css`에만 작성합니다.
...
<section class="pl-section pl-[section-type]" data-section="[type]">
  <div class="pl-section__container pl-[section-type]__container">
    <!-- 섹션 타이틀 (반드시 Web Component 사용!) -->
    <pl-section-title label="Section Label" heading="Section Title"></pl-section-title>
...
#### 섹션 HTML 파일 필수 포함 항목 체크리스트 ⭐

새 섹션 HTML 파일에는 다음이 모두 포함되어야 합니다:
...
> 💡 **`npm run new-section`** 명령어를 사용하면 위 항목이 자동으로 포함된 보일러플레이트 파일이 생성됩니다.
...
| 속성 | 필수 | 설명 |
|------|------|------|
| `label` | ❌ | 상단 라벨 텍스트 (예: "EVENT BENEFITS") |
| `heading` | ✅ | 메인 타이틀 텍스트 |
| `description` | ❌ | 부가 설명 (HTML `<br>` 태그 사용 가능) |
| `note` | ❌ | 하단 참고 문구 (예: "*신규 회원 한정") |
...
### 7.5 Button (Web Component)

버튼은 `<pl-button>` Web Component를 사용합니다.

#### 기본 사용법
```html
<!-- 일반 버튼 -->
<pl-button variant="primary" size="large">무료 체험 시작하기</pl-button>

<!-- 링크 버튼 -->
<pl-button variant="outline" href="#">자세히 보기</pl-button>

<!-- 비활성화 버튼 -->
<pl-button variant="primary" disabled>신청 마감</pl-button>
```
...
| `size` | - | `large` / `small` (미지정 시 medium) |
| `href` | - | 지정 시 `<a>` 태그로 렌더링, 미지정 시 `<button>` |
| `type` | `button` | `submit` / `reset` (`<button>`에만 적용) |
| `disabled` | - | 비활성화 |
| `full` | - | 너비 100% |
| `icon-only` | - | 아이콘 전용 버튼 |
| `aria-label` | - | 접근성 레이블 |
...
#### ⚠️ CTA 섹션 예외
CTA 섹션(`sections/cta/`)의 특수 버튼은 `pl-btn pl-cta__btn` 클래스를 직접 사용합니다. 이 경우에는 `<pl-button>` 컴포넌트를 사용하지 않습니다.

```html
<!-- CTA 섹션 전용 버튼 패턴 (예외) -->
<a href="#" class="pl-btn pl-cta__btn pl-cta__btn--filled">무료 체험 신청</a>
```

---

### 7.6 Card (Web Component)

카드 컴포넌트는 `<pl-card>` Web Component를 사용합니다.
...
<section class="pl-section pl-feature" data-section="feature">
  <div class="pl-feature__container">
    <!-- 섹션 타이틀 (반드시 Web Component 사용) -->
    <pl-section-title label="Features" heading="주요 기능"></pl-section-title>

    <!-- 기능 그리드 -->
    <div class="pl-feature-grid">
      <!-- pl-card 컴포넌트 또는 직접 마크업 -->
      <pl-card title="기능 1" description="설명" image="icon1.svg" image-alt="Feature 1"></pl-card>
      <!-- 더 많은 카드... -->
    </div>
  </div>
</section>
...
- `sections/about/type-g-video.html` 파일 자동 생성
- 필수 CSS 링크, components.js, 프리뷰 헤더, 빈 섹션 구조 포함
- 지원 타입: `hero` / `about` / `benefit` / `review` / `cta` / `stats` / `faq` / `form` / `gnb` / `etc`

### 14.2 섹션 HTML 구조 검증

```bash
npm run validate                                              # 전체 검사
npm run validate -- --file=sections/hero/type-a-split.html   # 단일 파일
```

검사 항목:
- 🔴 **심각** (반드시 수정): `tokens/base.css` / `sections.css` / `dark-mode.css` / `components.js` 링크 누락, `pl-section` 클래스 누락, `data-section` 속성 누락
- 🟡 **경고** (권장 수정): `<pl-section-title>` 컴포넌트 미사용, 컨테이너 클래스 누락, 인라인 다크모드 잔존, 하드코딩된 hex 색상
- 🔵 **정보** (개선 제안): img alt 텍스트 없음, `<pl-button>` 미사용, Pretendard 폰트 누락

### 14.3 토큰 검증

```bash
npm run tokens:validate   # CSS에서 undefined var(--pl-*) 참조 탐지
npm run tokens:sync       # 토큰 재생성 + 검증 한 번에
```
